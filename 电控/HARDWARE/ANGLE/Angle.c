...
void UART4_IRQHandler(void)                	//串口1中断服务程序
{
	if (USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)
	{
		...
		if(uart4_packet[0] != 0x55) uart4_p=0;
		if(uart4_p >= 11)
		{
			uart4_p = 0;
			Read_Uart4(uart4_packet[1]);
		}
...
void Read_Uart4(u8 TYPE_Data)
{
	...
	if(TYPE_Data == 0x53)
	{	
		Roll_L  = uart4_packet[2];
		Roll_H  = uart4_packet[3];
		Pitch_L = uart4_packet[4];
		Pitch_H = uart4_packet[5];
		Yaw_L   = uart4_packet[6];
		Yaw_H   = uart4_packet[7];
		Roll_Data =(int16_t)(Roll_H<<8|Roll_L);
		Pitch_Data=(int16_t)(Pitch_H<<8|Pitch_L);
		Yaw_Data  =(int16_t)(Yaw_H<<8|Yaw_L);
		Roll_1 = ((float)Roll_Data) * 180 / 32768;
		Pitch_1 = ((float)Pitch_Data) * 180 / 32768;
		Yaw_1 = ((float)Yaw_Data) * 180 / 32768;
	}
	else {};
}
...
//		if(uart5_packet[0] != 0x55) uart5_p=0;
		if(uart5_p >= 11)
		{
			uart5_p = 0;
...
//		Roll_L  = uart5_packet[2];
//		Roll_H  = uart5_packet[3];
//		Pitch_L = uart5_packet[4];
//		Pitch_H = uart5_packet[5];
//		Yaw_L   = uart5_packet[6];
//		Yaw_H   = uart5_packet[7];
		Roll_Data =(int16_t)(Roll_H<<8|Roll_L);
		Pitch_Data=(int16_t)(Pitch_H<<8|Pitch_L);
		Yaw_Data  =(int16_t)(Yaw_H<<8|Yaw_L);
		Roll_2 = ((float)Roll_Data) * 180 / 32768;
		Pitch_2 = ((float)Pitch_Data) * 180 / 32768;
		Yaw_2 = ((float)Yaw_Data) * 180 / 32768;
	}
	else {};
}
